...
typedef enum
{
    ENEMY_ID,					//這個是enemy1.txt裡面的ID
    ENEMY_TEMPNO,			//這個是enemybase1.txt裡面的ID
    ENEMY_LV_MIN,
    ENEMY_LV_MAX,
    ENEMY_CREATEMAXNUM,
    ENEMY_CREATEMINNUM,
...
typedef struct tagENEMYTEMP_Table
{
    ...
    //吉仔 用於優化搜索循環
		int					enemyid;				//EnemyTable的 ENEMY_ID
}ENEMYTEMP_Table;
...
#endif
#ifdef _SAF_PET_EVO
int Enemy_Evolution(int petindex, int charaindex,int evobaseid, int image);
#endif
#endif
